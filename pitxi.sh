...
y/n ";;
  ...
  if [ ! -f .credentials ] || [ "$1" == "useNewCredentials" ]; then
    ...
    cp .credentials .credentials_tmp
  fi
  echo "
Connecting to Paypal and downloading missing files
  ...
  expect {
    "sftp>"     { exp_continue }
    timeout     {puts "*** Connection timed out. Check connection and verify the username/password are correct"; exit 1}
  }
  ...
  puts "Comparing with files in /PPLCSVFiles"
	sleep 1
	#;send "!diff -aw --changed-group-format='%<' --unchanged-group-format='' .RemoteFileList.txt .LocalFileList.txt > missingfiles.txt\n"
...
if [ ! -f .credentials ] || [ "$1" == "useNewCredentials" ]; then
  while true; do
  read -p "Do you want to save these login details for next time?
y/n " yn
    case $yn in
      [Yy]* )
      ...
      [Nn]* )
        ...
        echo "these credentials were not saved."
        exit 1
        ;;
      * ) echo "Please answer yes or no.
y/n ";;
